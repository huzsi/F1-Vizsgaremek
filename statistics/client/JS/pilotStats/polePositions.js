...
async function fetchDataPolePositions(tableId) {
    try {
      const response = await fetch("/api/polePositions"); // Az API végpont
    ...
    // Adatok hozzáadása a táblázathoz
        ...
        for (let i = currentIndex; i < endIndex; i++) {
            ...
            <td>${drivers.polePositionCount}</td>
            `;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const button = document.getElementById("loadPolePositions");
    button.addEventListener("click", () => {
      fetchDataPolePositions("dataTable");
