...
async function fetchdataConstructorPolePosition(tableId) {
    try {
      const response = await fetch("/api/constructorPolePosition"); // Az API végpont
    ...
    // Adatok hozzáadása a táblázathoz
        ...
        for (let i = currentIndex; i < endIndex; i++) {
            ...
            row.innerHTML = `
            <td>${constructors.constructor_name}</td>
            <td>${constructors.pole_positions}</td>
            `;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const button = document.getElementById("loadConstructorPolePositions");
    button.addEventListener("click", () => {
      fetchdataConstructorPolePosition("dataTable");
